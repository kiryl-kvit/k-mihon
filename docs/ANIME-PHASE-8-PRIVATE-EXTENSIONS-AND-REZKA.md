# Anime Phase 8: Private Extensions and Rezka
...
- The Rezka package should expose multiple `AnimeCatalogueSource` entries via `AnimeSourceFactory` rather than force all sections through one overloaded source.
...
- anime extensions can already load and register into `AnimeSourceManager`
...
Use one Rezka extension package that exposes multiple sources via `AnimeSourceFactory`.
...
- [ ] Add the Rezka extension package and `AnimeSourceFactory`
...
- [ ] Parse title details into `SAnime`
...
- verify the app loads the installed anime extension into `AnimeSourceManager`
...
- the package exposes all four Rezka sections through `AnimeSourceFactory`
