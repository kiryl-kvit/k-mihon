# Anime Phase 4: Built-In Player Vertical Slice
...
- the player resolves a real `VideoStream` through `AnimeSourceManager`
...
- `source-api/src/commonMain/kotlin/eu/kanade/tachiyomi/source/AnimeSource.kt`
...
- `app/src/main/java/eu/kanade/tachiyomi/source/AndroidAnimeSourceManager.kt`
...
- Resolve the source from `AnimeSourceManager` using `AnimeTitle.source`.
...
- An `AnimeTitle -> SAnime` mapper can stay deferred unless a concrete player need appears during implementation.
...
- the player resolves and plays a real stream through `AnimeSourceManager`
...
- Phase 2 and Phase 3 prerequisites remain the underlying base: `AnimeSourceManager`, `VideoRequest`, `VideoStream`, playback-state persistence, and anime history storage

## Handoff To Phase 5

Phase 5 planning now lives in `docs/ANIME-PHASE-5-UI.md`.
