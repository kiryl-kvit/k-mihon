# Anime Phase 2: Source and Extension Runtime Foundation
...
- anime runtime source registration lives in a new `AnimeSourceManager`
...
- `AnimeSourceManager` is added in parallel
- `AnimeCatalogueSource` remains fully parallel to manga `CatalogueSource`
...
- register only into `AnimeSourceManager`
...
- add `AnimeSource` / `AnimeSourceFactory` path for anime packages
- compute package language/display metadata from the type-appropriate runtime payload

### Files

- `app/src/main/java/eu/kanade/tachiyomi/extension/util/ExtensionLoader.kt`
- `source-api/src/commonMain/kotlin/eu/kanade/tachiyomi/source/AnimeSourceFactory.kt`
- `source-api/src/commonMain/kotlin/eu/kanade/tachiyomi/source/AnimeCatalogueSource.kt`
...
- new `domain/.../AnimeSourceManager.kt`
- `app/src/main/java/eu/kanade/tachiyomi/source/AndroidSourceManager.kt`
- new `app/.../source/AndroidAnimeSourceManager.kt`
...
- `AnimeSourceManager` should be parallel, even if its first implementation is narrower than manga’s
- video stub-source persistence is deferred unless it becomes necessary for fallback labels in this phase

### Checklist

- [x] Add `AnimeSourceManager` interface
- [x] Add runtime `AndroidAnimeSourceManager`
- [x] Keep manga source manager registration unchanged for manga packages
- [x] Register anime packages into `AnimeSourceManager` only
...
- add an app-side `AnimeSourcePreferenceProvider`
...
- `source-api/src/commonMain/kotlin/eu/kanade/tachiyomi/source/ConfigurableAnimeSource.kt`
- `source-api/src/commonMain/kotlin/eu/kanade/tachiyomi/source/AnimeSourcePreferenceProvider.kt`
...
2. Loader branching and `AnimeSourceManager`
...
- verify anime packages register only in `AnimeSourceManager`
...
- anime runtime registration now flows through a dedicated `AnimeSourceManager`
- extension visibility initialization is now split across manga and video hidden-source keys
- anime configurable-source preferences now use a separate profile-scoped `anime_source_<profileId>_<sourceId>` namespace
