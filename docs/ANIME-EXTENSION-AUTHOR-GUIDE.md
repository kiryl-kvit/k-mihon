...
```kotlin
class MyAnimeSourceFactory : AnimeSourceFactory {
...
- anime source prefs: `anime_source_<profileId>_<sourceId>`

Implications:

- manga and video source IDs do not share the same preference store name
- preferences are still profile-scoped
- configurable anime sources should use `ConfigurableAnimeSource` and `animeSourcePreferences()` rather than trying to access manga source preferences directly
...
- new anime repos should always emit `type: "anime"`

## Current Runtime Scope

At the current implementation state:

- anime extensions are loadable and register into `AnimeSourceManager`
...
- add `tachiyomi.extension.type = anime`
...
- expose `AnimeSource` or `AnimeSourceFactory` only
...
- use `ConfigurableAnimeSource`
- use `videoSourcePreferences()` or `videoPreferenceKey()`

## Author Checklist

- [ ] Add `tachiyomi.extension.type = anime`
- [ ] Use `AnimeSource` or `AnimeSourceFactory`
- [ ] Do not export manga `Source` runtime classes from a video package
- [ ] Use `ConfigurableAnimeSource` if the source has preferences
- [ ] Ensure repo index entries include `type: "anime"`
