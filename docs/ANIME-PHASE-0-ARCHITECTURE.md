# Anime Phase 0: Architecture Lock
...
- `AnimeCatalogueSource : AnimeSource`
...
- `ConfigurableAnimeSource : AnimeSource`
...
- The first API revision mirrors configurable-source support for anime via `ConfigurableAnimeSource`.
...
- add a parallel `AnimeSourceManager` for registered anime sources
...
- anime packages instantiate `AnimeSource` / `AnimeSourceFactory`
...
- add a parallel `AnimeSourceManager` rather than broadening the current interface into a mixed media manager
...
- keep source-side models (`SAnime`, `SEpisode`) separate from app/domain/data models (`AnimeTitle`, `AnimeEpisode`, etc.)
...
- Phase 0 work is complete and later execution has progressed through Phases 1, 2, 3, 4, 5, 6, and 7, with Phase 7 now complete at the current scope
