...
class VideoExtensionsScreenModel(
    preferences: GlobalSourcePreferences = Injekt.get(),
    basePreferences: BasePreferences = Injekt.get(),
    private val extensionManager: ExtensionManager = Injekt.get(),
    private val getVideoExtensions: GetVideoExtensions = Injekt.get(),
) : StateScreenModel<VideoExtensionsScreenModel.State>(State()) {
    ...
    fun searchQueryPredicate(query: String): (Extension) -> Boolean {
        ...
        return { extension ->
            subqueries.any { subquery ->
                ...
                when (extension) {
                    is Extension.InstalledVideo -> extension.sources.any { source ->
                        source.name.contains(subquery, ignoreCase = true) ||
                            source.id == subquery.toLongOrNull()
                    }

                    is Extension.AvailableVideo -> extension.sources.any {
    ...
    fun installExtension(extension: Extension.AvailableVideo) {
        screenModelScope.launchIO {
            extensionManager.installExtension(extension).collectToInstallUpdate(extension)
        }
    }

    fun updateExtension(extension: Extension.InstalledVideo) {
        screenModelScope.launchIO {
