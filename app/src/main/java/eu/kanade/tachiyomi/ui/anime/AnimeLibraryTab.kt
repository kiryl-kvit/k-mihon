    ...
    override fun Content() {
        ...
        Scaffold(
            ...
            when (val current = state) {
                AnimeLibraryScreenModel.State.Loading -> LoadingScreen(Modifier.padding(contentPadding))
                is AnimeLibraryScreenModel.State.Error -> EmptyScreen(
                    message = current.message,
                    modifier = Modifier.padding(contentPadding),
                )
                is AnimeLibraryScreenModel.State.Success -> {
                    if (current.animes.isEmpty()) {
                        EmptyScreen(
                        ...
                        AnimeLibraryContent(
                            ...
                            onContinueWatching = { animeId, episodeId ->
                                context.startActivity(
                                    VideoPlayerActivity.newIntent(
                                        context = context,
                                        animeId = animeId,
        ...
        LaunchedEffect(state) {
            if (state !is AnimeLibraryScreenModel.State.Loading) {
...
private fun AnimeLibraryContent(
    state: AnimeLibraryScreenModel.State.Success,
    ...
    LazyColumn(
        ...
        items(state.animes, key = { it.animeId }) { item ->
            Surface(
                ...
                Column(
                    modifier = Modifier
                        .fillMaxWidth()
                        .clickable { onOpenAnime(item.animeId) },
                ) {
                    ...
                    ListItem(
                        ...
                        trailingContent = {
                            if (item.primaryEpisodeId != null) {
                                FilledIconButton(
                                    onClick = { onOpenEpisode(item.animeId, item.primaryEpisodeId) },
